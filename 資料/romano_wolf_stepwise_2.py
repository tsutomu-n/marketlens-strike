...
def _wild_bootstrap_stats(
    ...
    """
    Wild bootstrap for heteroskedastic errors (Wu 1986, Liu 1988).
    Demeans excess under H0 (mean excess=0), multiplies residuals by random external weights.
    Preserves the observed heteroskedasticity pattern (e.g. vol clustering) while enforcing the null.
    Excellent for financial returns with strong heteroskedasticity.
    Can be used standalone or combined with stationary (wild block bootstrap is advanced future work).
    """
    if rng is None:
        rng = np.random.default_rng()
    T, m = excess.shape
    excess_demeaned = excess - np.nanmean(excess, axis=0, keepdims=True)
    boot_stats = np.empty((n_boot, m))
    for b in range(n_boot):
        if weights == "rademacher":
            w = rng.choice([-1.0, 1.0], size=T)
        elif weights == "gaussian":
            w = rng.normal(0.0, 1.0, size=T)
        elif weights == "mammen":
            # Mammen weights: mean≈0, var=1, slightly asymmetric
...
def romano_wolf_stepwise(
    ...
    bootstrap_method : {"stationary", "wild", "iid"}
        "stationary" (default): Politis & Romano stationary bootstrap (good for serial correlation).
        "wild": Wild bootstrap with external weights (excellent for heteroskedasticity / vol clustering).
        "iid": Simple with-replacement (fast but ignores both serial corr and hetero).
        For financial returns (both serial + hetero present), "stationary" or "wild" is strongly preferred over "iid".
        Advanced: wild + block (stationary wild) is future extension.
    mean_block_length : float
        Only used for "stationary". Mean geometric block length. Daily: 10-30 typical.
    wild_weights : str
        Only used for "wild". "rademacher" (default, ±1), "gaussian", or "mammen".
    ...
    if bootstrap_method == "iid":
        ...
        boot_stats = _wild_bootstrap_stats(
