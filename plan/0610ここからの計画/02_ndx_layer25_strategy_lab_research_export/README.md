...
Plan review note: this plan was revised after checking the current Strategy Lab command flow. Signal-level `block_reasons` are not sufficient unless `build-paper-candidate-pack` explicitly propagates them into `TradeCandidate.block_reasons`; this propagation is now part of the implementation plan.
