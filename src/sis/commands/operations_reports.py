    ...
    *,
    ...
    def operations_audit_pack_cmd() -> None:
        ...
        chain_out = append_operations_audit_snapshot_manifest_fn(
            ...
            manifest_path=manifest_out,
            overall_status=payload.get("overall_status") if isinstance(payload, dict) else None,
            timeline_latest_operation=payload.get("timeline_latest_operation")
    ...
    def audit_bundle_cmd() -> None:
        ...
        audit_timeline_out, audit_timeline_summary_out, _ = write_audit_timeline_fn(
            settings.data_dir
        )
        audit_dashboard_out, audit_dashboard_summary_out, _ = write_audit_dashboard_fn(
            settings.data_dir
        )
        out, manifest_out, text = write_audit_bundle_fn(settings.data_dir)
        audit_bundle_history_out, audit_bundle_history_summary_out, _ = (
            write_audit_bundle_history_fn(settings.data_dir)
        )
        logger.info("written: {}", out)
        logger.info("written: {}", manifest_out)
