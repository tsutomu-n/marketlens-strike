...
def _execution_price(row: dict) -> float | None:
...
def _exit_price(row: dict) -> float | None:
...
def run_backtest_bridge(quotes_path: Path) -> list[BacktestMetrics]:
...
def write_backtest_report(metrics: list[BacktestMetrics], out_path: Path) -> None:
    ...
    out_path.write_text(
        "\n".join(
            [
                ...
                "",
                "This report uses venue quote logs for virtual execution. It is not a trading signal generator.",
                "",
