...
def build_audit_timeline_report(
    *,
    ...
    diagnostics_status_counts = note_counts(audit_ops, "execution_diagnostics_status=")
    drift_overview_status_counts = note_counts(audit_ops, "execution_drift_overview_status=")
    ...
    gap_history_status_counts = note_counts(audit_ops, "execution_gap_history_latest_status=")
    ...
    readiness_next_phase_counts = note_counts(audit_ops, "readiness_next_phase=")
    readiness_execution_ready_counts = note_counts(audit_ops, "readiness_execution_ready=")
    latest = recent[-1] if recent else {}
    latest_notes = latest.get("notes", []) if isinstance(latest, dict) else []
    if not isinstance(latest_notes, list):
        latest_notes = []
    phase_gate_decision_counts = note_counts(audit_ops, "phase_gate_decision=")
    phase2_entry_allowed_counts = note_counts(audit_ops, "phase2_entry_allowed=")
    phase_gate_reason_counts = note_counts(audit_ops, "phase_gate_reason=")
    ...
    phase_gate_checked_files_values = note_counts(audit_ops, "phase_gate_checked_files=")
    ...
    latest_note_summary = timeline_latest_note_summary(
        execution_notes=latest_notes,
        readiness_notes=latest_notes,
        phase_gate_notes=latest_notes,
    )
    ...
    summary = {
        ...
        **remediation_summary,
        ...
        "operations_audit_pack_report_path": (
            str(reports_dir_value / "operations_audit_pack.md") if reports_dir_value else None
        ),
        ...
        "audit_bundle_history_report_path": (
            str(reports_dir_value / "audit_bundle_history.md") if reports_dir_value else None
        ),
    ...
    text = render_audit_timeline_markdown(summary=summary, recent=recent)
