...
def _quick_navigation(
    ...
    items: list[tuple[str, str | None]] = [
        ...
        (
            ...
            payload.get("live_evidence_report_path") if isinstance(payload.get("live_evidence_report_path"), str) else None,
        ),
    ]
    return {key: value for key, value in items if isinstance(value, str) and value}


def _nested_report_path(payload: dict[str, object], section: str, flat_key: str) -> str | None:
...
def _related_reports(out_path: Path | None, payload: dict[str, object]) -> dict[str, str]:
    if out_path is None:
        return {}
    reports_dir = out_path.parent
    items: list[tuple[str, str | None]] = [
