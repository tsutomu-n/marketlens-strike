...
class CaptureManifest(BaseModel):
    ...
    schema_version: Literal["crypto_perp_capture_manifest.v1"] = CAPTURE_MANIFEST_SCHEMA_VERSION
    artifact_id: str
    created_at: datetime
...
def run_candidate_capture(
    ...
    *,
    ...
    targets = [
        BitgetWsTarget(inst_type="USDT-FUTURES", channel=channel, inst_id=request.native_symbol)
        for channel in request.channels
    ]
    subscription_message = build_subscribe_message(targets)
    _ = subscription_message
    rows: list[dict[str, Any]] = []
    books: dict[tuple[str, str], BitgetOrderBook] = {}
    known_gaps: list[KnownCaptureGap] = []
    ...
    for raw in raw_messages:
        ...
        if parsed.kind != "data" or parsed.channel not in {"books", "books1", "books15"}:
            continue
        key = _book_key(parsed.channel, parsed.native_symbol or request.native_symbol)
        ...
        for item in parsed.data:
            ...
            result = book.apply_depth(
                ...
                seq=seq,
                checksum=checksum,
                ts_event_ms=parsed.ts_event_ms or 0,
            )
            ...
            if result.invalid_reason is not None:
                known_gaps.append(
                    KnownCaptureGap(
                        ...
                        native_symbol=parsed.native_symbol or request.native_symbol,
                        reason_code=result.invalid_reason,
                        ts_event=ts_event,
                        seq=seq,
                    )
                )
    capture_id = _capture_id(request, received)
    segment_refs = []
    if rows:
        segment_refs.append(
            write_gzip_jsonl_segment(
                request.output_root / f"event_id={request.event_id}" / "part-000001.jsonl.gz",
                rows,
            )
        )
    coverage_status: Literal["COMPLETE", "GAPPED", "EMPTY", "FAILED"]
    ...
    return CaptureManifest(
        artifact_id=stable_hash(["crypto-perp-capture-manifest-artifact", capture_id]),
        created_at=received,
        producer=CryptoPerpProducer(command="crypto-perp-capture"),
        source_refs=[],
