...
def test_block_trade_row_neutralizes_trade_fields_and_appends_block_reason(tmp_path) -> None:
    ...
    trade_row = _trade_signal_row(
        ...
        raw_score=0.75,
        rank=0.75,
    ) | {"block_reasons": ["existing_block"]}

    row = _block_trade_row(
        trade_row,
        spec=spec,
    ...
    assert row["signal_id"] != trade_row["signal_id"]
    ...
    assert row["block_reasons"] == ["existing_block", "risk_throttle_daily_loss"]
