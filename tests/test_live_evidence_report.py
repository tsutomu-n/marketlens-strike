...
def test_build_and_render_live_evidence_report(tmp_path) -> None:
    ...
    assert data.readiness_summary["readiness_next_phase_candidate"] == "Stay Phase 1"
    assert data.readiness_summary["readiness_execution_ready"] is False
    assert data.timeline_latest_execution_summary["execution_overall_status"] == "ok"
    assert (
    ...
    assert data.bundle_history_latest_execution_summary["execution_overall_status"] == "warn"
    assert (
    ...
    assert data.cycle_history_latest_execution_summary["execution_overall_status"] == "ok"
    assert (
    ...
    assert "<h2>Readiness Summary</h2>" in html_text
    assert "<h2>Latest Execution Lineage</h2>" in html_text
    ...
    assert "## Quick Navigation" in followup_text
    assert "## Related Reports" in followup_text
    assert "live_evidence_followup_report:" in followup_text
    assert "phase_gate_review_report:" in followup_text
    assert "overall_status: `ok`" in followup_text
    assert "## Phase Gate Summary" in followup_text
    assert "## Readiness Summary" in followup_text
    assert "## Latest Execution Lineage" in followup_text
