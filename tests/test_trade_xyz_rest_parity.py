...
class _FakeClient:
    ...
    def meta_and_asset_ctxs(self):
        return {"universe": [{"name": "xyz:SP500"}]}, [{}]
...
class _FakeClientWithOptionalEndpointGaps:
    def all_mids(self):
        return {"xyz:SP500": "100.0"}
...
def test_build_trade_xyz_rest_parity_manifest_treats_optional_endpoint_errors_as_known_gaps(
    tmp_path: Path,
) -> None:
    data_dir = tmp_path / "data"
    ws_manifest_path = data_dir / "manifests/trade_xyz_ws_capture_manifest.json"
    ws_manifest_path.parent.mkdir(parents=True, exist_ok=True)
    ws_manifest_path.write_text(json.dumps({"symbols": ["SP500"]}), encoding="utf-8")
    manifest = build_trade_xyz_rest_parity_manifest(
        data_dir=data_dir,
        ws_manifest_path=ws_manifest_path,
        symbols=["SP500"],
        client=_FakeClientWithOptionalEndpointGaps(),
        request_delay_seconds=0.0,
        include_l2_book=False,
    )
    assert manifest["status"] == "pass"
    assert manifest["request_error_count"] == 0
    assert manifest["known_gap_count"] == 3
