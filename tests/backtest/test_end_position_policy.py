...
from sis.backtest.engine.end_position_policy import BacktestError, apply_end_position_policy
...
def _config(policy: str) -> BacktestConfig:
    return BacktestConfig(
        run_id="end-position-policy",
...
def test_apply_end_position_policy_error_if_open_raises() -> None:
    with pytest.raises(BacktestError, match="open position at end of run"):
        apply_end_position_policy(
            config=_config("error_if_open"),
            rows=[_row()],
...
def test_apply_end_position_policy_mark_to_market_only_is_noop() -> None:
    orders: list[Order] = []
    fills: list[Fill] = []
    ...
    next_portfolio, next_open_trade = apply_end_position_policy(
        config=_config("mark_to_market_only"),
        rows=[_row()],
        portfolio=portfolio,
    ...
    assert blocked == []
    assert trades == []
    assert equity_rows == [{"position_qty": 3.0, "equity": 9_300.0}]


def test_apply_end_position_policy_force_close_if_executable_delegates_close() -> None:
    orders: list[Order] = []
    fills: list[Fill] = []
    blocked: list[BlockedEvent] = []
    trades: list[dict[str, object]] = []
    ...
    portfolio, open_trade = apply_end_position_policy(
