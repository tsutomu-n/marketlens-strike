...
- NDX/QQQ family は research record と backtest input として保持できるが、現行の Strategy Lab paper path では `selected_candidate_ids`、`PaperIntentPreview`、raw JSON の `paper-from-intents`、legacy `paper-step` order generation で fail closed する。
...
- NDX/QQQ family records remain usable for research/backtest artifacts, but paper candidate selection, `PaperIntentPreview`, raw `paper-from-intents` JSON, and legacy `paper-step` order generation are blocked until a future residual-validation and operator-promotion path changes that.
