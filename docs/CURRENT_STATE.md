...
- Crypto Perp Truth-Cycle MVP は M05 時点で Bitget public provider probe、immutable raw snapshot、universe diff、ticker market snapshot、15m candle finality、gap / non-final bar quality guard、slow / fast / near-miss event detection、direction-neutral event artifact、event card rendering、candidate-only raw-first WS capture manifest、book checksum / sequence guard、atomic gzip JSONL segments までです。M06 以降の decision / outcome surface はまだ未実装です。
