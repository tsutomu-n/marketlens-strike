...
- Strategy Lifecycle control plane は `strategy-backtest-acceptance`、`strategy-paper-observation-cycle`、`strategy-lifecycle-review` に実装済み。Backtest acceptance、fresh paper intent generation、session manifest / session ledger、NDX paper observation review、phase gate summary を local artifact で統合し、`REJECT_OR_REVISE` / `CONTINUE_RESEARCH` / `BACKTEST_ACCEPTED` / `CONTINUE_PAPER_OBSERVATION` / `CONTINUE_EXECUTION_READINESS` / `ELIGIBLE_FOR_LIVE_CANARY_PLAN` / `BLOCKED_BOUNDARY_VIOLATION` を出す。これは live order、wallet、exchange write を許可しない。`strategy-backtest-suite` は `strategy_backtest_suite.v1` YAML から複数spec / 複数backtest case を実行し、suite result / report に集約し、標準例では `single_window`、`walk_forward:trading_day`、`purged_walk_forward:trading_day`、`purged_walk_forward:trading_day+return_bootstrap`、`purged_walk_forward:trading_day+block_bootstrap` の手法別 run 数を `method_matrix` に記録する。resampling case は実行済み signal return から deterministic bootstrap 分布を作り、`summary.resampling` に p05 / p50 / p95 / positive rate を残す。`strategy-backtest-stress` は既存 backtest metrics の executed signal return に追加 cost / slippage bps を掛け、scenario 別 robustness result を paper-only artifact に記録する。`strategy-backtest-regime-split` は既存 backtest metrics の executed signal return を dimension 別に集計し、弱い bucket を paper-only artifact に記録する。`strategy-backtest-rolling-stability` は既存 backtest metrics の executed signal return を rolling window 別に集計し、弱い期間を paper-only artifact に記録する。`strategy-backtest-compare` は現行 backtest metrics を比較用 canonical artifact に正規化し、native overall / walk-forward era / optimizer sweep の method result、suite result、suite method matrix、threshold failure / weakest era / suite best run 診断、外部 framework adapter 候補の状態、既存 adapter spike artifact、既存 external framework result、既存 portfolio comparison result、既存 metric extension result、既存 report extension result、既存 stress result、既存 regime split result、既存 rolling stability result を同じ artifact に記録する。`strategy-backtest-pack` は単発 Strategy Authoring backtest、5手法 suite、bundle result、adapter spike、external result、portfolio comparison、metric extension、report extension、cost / slippage stress、regime split、rolling stability、comparison、pack manifest を一括生成し、artifact path / hash、no-live boundary、`external_framework_policy` を記録する。標準 engine は `strategy_authoring_native` で、標準完成線は `complete_without_locked_external_dependency`、`locked_dependency_added=false`、`external_adapters_required_for_completion=false` である。`strategy-backtest-pack-validate` は pack manifest の artifact path / hash、標準5手法、paper-only / no-live boundary、外部 framework 方針を検査し、PASS / FAIL artifact を作る。`strategy-backtest-adapter-spike` は `vectorbt`, `bt`, `backtesting.py`, `zipline-reloaded`, `backtrader`, `quantstats`, `empyrical-reloaded`, `pyfolio-reloaded`, `qstrader` の import / metadata / license risk を dependency-free artifact に記録する。`strategy-backtest-framework-smoke` は一時 `uv --with ...` 環境で `vectorbt`, `bt`, `quantstats`, `empyrical-reloaded` の import status、version、license metadata、Requires-Python、採用分類を dependency-free artifact に記録する。`strategy-backtest-adapter-selection` は `vectorbt`, `bt`, `empyrical-reloaded`, `quantstats` を adapter design 候補として selected にし、その他候補を deferred にする。`strategy-backtest-adapter-contract` は selected adapter の入力、出力、provenance、受入条件を dependency-free artifact に記録する。`strategy-backtest-external-run` は外部 framework 候補の run status、framework version、runner mode を dependency-free artifact に記録し、`vectorbt` がインストール済みで inputs がある場合は `src/sis/backtest/vectorbt_adapter.py` 経由で `vectorbt.Portfolio.from_signals` を呼ぶ。`strategy-backtest-portfolio-compare` は `bt` 用の portfolio allocation / rebalance comparison artifact を記録し、`bt` がインストール済みで inputs がある場合は `src/sis/backtest/portfolio_comparison.py` 経由で `bt.run()` を呼ぶ。`strategy-backtest-metric-extension` は `empyrical-reloaded` 用の metrics normalization artifact を記録し、`empyrical` がインストール済みで returns series を作れる場合は `src/sis/backtest/metric_extension.py` 経由で `empyrical` metric を呼ぶ。`strategy-backtest-report-extension` は `quantstats` 用の report / tear sheet artifact を記録し、`quantstats` がインストール済みで returns series を作れる場合は `src/sis/backtest/report_extension.py` 経由で `quantstats.reports.html` と `quantstats.reports.metrics` を呼ぶ。`uv run --with vectorbt`、`uv run --with bt`、`uv run --with empyrical-reloaded`、`uv run --with quantstats` の一時 smoke は通過済み。live 許可は出さない。
...
- Strategy backtest comparison, suite, adapter spike, external result, portfolio comparison, metric extension, report extension, stress, regime split, rolling stability, pack, and pack validation commands: `strategy-backtest-compare`, `strategy-backtest-suite`, `strategy-backtest-adapter-spike`, `strategy-backtest-external-run`, `strategy-backtest-portfolio-compare`, `strategy-backtest-metric-extension`, `strategy-backtest-report-extension`, `strategy-backtest-stress`, `strategy-backtest-regime-split`, `strategy-backtest-rolling-stability`, `strategy-backtest-pack`, `strategy-backtest-pack-validate`
