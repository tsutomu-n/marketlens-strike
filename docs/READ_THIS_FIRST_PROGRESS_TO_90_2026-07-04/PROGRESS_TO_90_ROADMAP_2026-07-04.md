...
| Human decision | kill / revise / collect_more / future_hold を判断記録として残せる |
...
これ以外の拡張は、actual cash blocker を将来扱う段階まで後回しでよい。当面は pre-actual-cash decision gate で候補の扱いを決める。
...
長期的には、Reality Check の最大 blocker である `ACTUAL_CASH_SOURCE_MISSING` を解消し、利益判断の入口を before-cost / estimate から actual cash basis へ進める。当面は actual cash source を作らず、pre-actual-cash decision gate で候補を棄却・再定義・追加収集・将来保留に分類する。
...
長期で actual cash を再開する場合の最短順は次。当面の実行順は、この表ではなく pre-actual-cash decision gate を優先する。
...
ただし、当面 actual cash を実装しない方針では、これを次の実装対象にしない。短期の実装対象は追加CLIやactual cash sourceではなく、既存 artifact を読んで候補を4択 decision に落とす運用境界である。
