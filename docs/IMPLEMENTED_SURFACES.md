...
| Strategy Workbench Viewer first slice | implemented as static HTML artifact viewer, including Crypto Perp tournament gate, truth-cycle status, next-step, and first-stage-blocker summaries, no paper/live permission | `strategy-workbench-viewer-build`, `src/sis/strategy_workbench_viewer/`, `schemas/strategy_workbench_viewer.v1.schema.json`, `tests/strategy_workbench_viewer/`, `docs/strategy_workbench_viewer/` |
