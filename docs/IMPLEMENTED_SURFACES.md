...
この文書は現行コードで使える主要 surface を読むための入口です。技術寄りの capability index は [REPO_CAPABILITIES_CURRENT_2026-06-16.md](REPO_CAPABILITIES_CURRENT_2026-06-16.md) を読む。実務的な次方向は [CURRENT_GOAL_AND_DIRECTION_2026-07-05.md](CURRENT_GOAL_AND_DIRECTION_2026-07-05.md) を読む。
...
| Profit-readiness local automation | implemented as local inventory / planner / chain runner / actual-cash conversion / review packet / readiness artifacts; blocks missing real event, matured outcome, or ledger evidence | `crypto-perp-profit-readiness-inventory`, `crypto-perp-profit-readiness-plan`, `crypto-perp-profit-readiness-run-local`, `crypto-perp-cash-ledger`, `crypto-perp-actual-cash-rows-build`, `crypto-perp-actual-cash-report-gate`, `crypto-perp-tiny-live-review-packet`, `crypto-perp-tiny-live-shadow-readiness`, `src/sis/crypto_perp/profit_readiness.py`, `schemas/crypto_perp_profit_readiness_*.schema.json`, `schemas/crypto_perp_actual_cash_*.schema.json`, `schemas/crypto_perp_tiny_live_review_packet.v1.schema.json`, `schemas/crypto_perp_tiny_live_shadow_readiness.v1.schema.json`, `tests/crypto_perp/test_profit_readiness_local_automation.py` |
| Backtest candidate pack | implemented as local timestamp-safe simulation evidence pack; classifies candidates without actual cash and never promotes to live | `crypto-perp-backtest-candidate-pack`, `src/sis/crypto_perp/backtest_candidate_pack.py`, `src/sis/crypto_perp/backtest_candidate_pack_models.py`, `src/sis/crypto_perp/backtest_candidate_pack_reports.py`, `schemas/crypto_perp_backtest_candidate_pack.v1.schema.json`, `tests/crypto_perp/test_backtest_candidate_pack.py` |
| Pre-actual-cash evidence pack gate | implemented as internal local multi-event/outcome aggregation and 4-choice candidate classification, not profit proof; no public CLI | `src/sis/crypto_perp/pre_actual_cash.py`, `schemas/crypto_perp_pre_actual_cash_decision.v1.schema.json`, `tests/crypto_perp/test_profit_readiness_local_automation.py` |
...
| Risk-taker review artifact | implemented as local human-risk-review classifier over rows v2, source availability, and bias guard; never grants live permission | `crypto-perp-risk-taker-review`, `src/sis/crypto_perp/risk_taker_review.py`, `schemas/crypto_perp_risk_taker_review.v1.schema.json`, `tests/crypto_perp/test_risk_taker_review.py` |
| Profit Core reality check | implemented as read-only blocker summary over existing candidate, C9 bridge, profit-readiness, risk review, and actual-cash artifacts; never generates missing artifacts or grants live permission | `profit-core-reality-check`, `src/sis/profit_core_reality_check/`, `schemas/profit_core_reality_check.v1.schema.json`, `tests/profit_core_reality_check/` |
...
- Crypto Perp tournament compares `REVERSAL_SHORT`, `CONTINUATION_LONG`, and `NO_TRADE` on the same event set. `cash_metric_value_usd` is the value column, while `actual_cash_result_usd` is an actual-cash-only legacy alias. `cash_metric_basis`, `primary_metric_display_name`, and `actual_cash` are the semantic guard. Insufficient data remains `INCONCLUSIVE_DATA`. Rows produced by `crypto-perp-tournament-rows-preview` are `cash_metric_basis=before_cost_proxy`, not actual cash evidence, and `crypto-perp-tournament-report` rejects preview / non-actual rows with `PREVIEW_ROWS_NOT_ACTUAL_CASH`.
- `crypto-perp-event-record` records `market_window_v1` events from already validated public candle CSV inputs. This is a real market observation window, not a pump-detector trigger, alpha proof, actual cash evidence, paper permission, or live permission.
...
- `crypto-perp-risk-taker-review` classifies an existing local candidate as `READY_FOR_HUMAN_RISK_REVIEW`, `NEEDS_ACTUAL_CASH`, `BLOCKED_BY_VENUE`, `INCONCLUSIVE_DATA`, or `KILL`. `READY_FOR_HUMAN_RISK_REVIEW` is not live, credential, exchange-write, tiny-live, or order permission.
- Crypto Perp profit-readiness local automation does not fetch missing data. Inventory/plan stops with `BLOCKED_MISSING_EVENT_OR_OUTCOME` when real event and matured outcome artifacts are absent, and stops with `BLOCKED_MULTIPLE_EVENT_OR_OUTCOME_CANDIDATES` rather than auto-selecting. Dogfood/status/viewer artifacts are not profit evidence. `crypto-perp-actual-cash-rows-build` only converts cash ledger plus explicit assignment to actual-cash rows; trade actions without ledger entries fail instead of zero-filling.
- `crypto-perp-backtest-candidate-pack` creates local simulation artifacts only: `signal_rows.jsonl`, data availability ledger, execution assumptions, no-lookahead report, backtest/stress/regime/rolling summaries, and a 4-choice decision. `BACKTEST_CANDIDATE_HOLD` is not live readiness, actual-cash readiness, tiny-live permission, paper permission, or profit proof.
- `profit-core-reality-check` reads explicit existing artifacts and chooses one deterministic blocker to fix. It does not run candidate generation, C9 bridge, actual-cash rows build, actual-cash gate, demo/testnet lifecycle, network calls, credential use, exchange write, or live orders.
