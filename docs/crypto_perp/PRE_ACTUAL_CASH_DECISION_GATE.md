...
既存の `crypto_perp_profit_readiness_run.v1` manifest がある場合、pack は `run_manifest.status` と `run_manifest.known_gap_count` を読み込む。manifest がない場合は `missing` として扱い、存在する run manifest 自体を未知スキーマや利益証拠として読まない。
