...
この gate の実装 surface は `src/sis/crypto_perp/pre_actual_cash.py` の内部 builder と `crypto_perp_pre_actual_cash_decision.v1` である。新しい public CLI は追加しない。これは local-only の集約と4択 decision だけを追加する。actual cash source、cash ledger、actual cash rows、actual-cash gate、live measurement、exchange write、wallet/signing、live order は追加しない。
...
内部 builder は、複数 event / outcome の pre-actual-cash evidence を集約し、internal writer で次の payload を指定出力先へ書ける。public CLI はない。
...
`decision.json` の `decision` は4択だけであり、`decision.md` は actual cash を扱っていないこと、利益証明ではないことを明示する。10 event / 10 outcome 未満でも pack は出せるが、v1 sample は不足として読む。

既存の `crypto_perp_profit_readiness_run.v1` manifest がある場合、pack は `run_manifest.status` と `run_manifest.known_gap_count` を読み込む。既存の `source_availability.json`、`replay_slice.json`、`feature_pack.json`、`edge_score.json`、`tournament_rows_v2.json`、`bias_guard.json` が `data_dir` 内にあり、対象 event / outcome pair と event set に紐づけて検証できる場合は、summary と decision の `source_gap_summary.artifact_usage` に `artifact_origin=existing`、path、`event_id`、`outcome_id` を載せる。既存 artifact がない場合は `artifact_origin=recomputed_minimal` とし、`artifact_gap_origin` に `minimal recomputed from event/outcome only` を明示する。manifest がない場合は `missing` として扱い、存在する run manifest 自体を未知スキーマや利益証拠として読まない。
...
- この gate / pack を actual cash 実装、actual cash readiness、tiny-live readiness、または live permission の根拠にする。
