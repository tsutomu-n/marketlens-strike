...
この gate の実装 surface は `crypto-perp-pre-actual-cash-evidence-pack` と `crypto_perp_pre_actual_cash_decision.v1` である。これは local-only の集約と4択 decision だけを追加する。actual cash source、cash ledger、actual cash rows、actual-cash gate、live measurement、exchange write、wallet/signing、live order は追加しない。
...
`crypto-perp-pre-actual-cash-evidence-pack` は、複数 event / outcome の pre-actual-cash evidence を集約し、次の出力を作る。
