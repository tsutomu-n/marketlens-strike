...
既存の `crypto_perp_profit_readiness_run.v1` manifest がある場合、pack は `run_manifest.status` と `run_manifest.known_gap_count` を読み込む。既存の `source_availability.json`、`replay_slice.json`、`feature_pack.json`、`edge_score.json`、`tournament_rows_v2.json`、`bias_guard.json` が `data_dir` 内にあり、対象 event set に紐づけて検証できる場合は、summary と decision の `source_gap_summary.artifact_usage` に `artifact_origin=existing` と path を載せる。既存 artifact がない場合は `artifact_origin=recomputed_minimal` とし、`artifact_gap_origin` に `minimal recomputed from event/outcome only` を明示する。manifest がない場合は `missing` として扱い、存在する run manifest 自体を未知スキーマや利益証拠として読まない。
