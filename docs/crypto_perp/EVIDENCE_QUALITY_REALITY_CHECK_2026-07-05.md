...
プロジェクト前提は taker fee 0.04% です。

一方で現行実装の backtest candidate pack default は 0.06% で、これは保守的な default と読めます。ただし、仕様として単一 source に寄せる必要があります。

参照設定は [../../configs/cost_models/crypto_perp_bitget_usdt_futures.yaml](../../configs/cost_models/crypto_perp_bitget_usdt_futures.yaml) に置きました。現時点では reference config であり、全 runtime builder が読む source of truth ではありません。
