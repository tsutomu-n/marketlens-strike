...
現在の30 events / 14 trades / 10 winsはnominal `+3.042366783076564551621614274 USD`、stress `+2.762366783076564551621614274 USD`です。naive trade iid bootstrap total 95%は`[+0.1069,+5.7882]`ですが、peak concurrent positions 6の依存を無視しています。5 episodes / 3 winsのepisode bootstrap total 95%は`[-1.9182,+9.2413]`、single-position totalは`-0.4618201695034107750204885438 USD`です。

always-longは`+5.816219911337534249441041925 USD`でselectorを上回り、score/result correlationは`-0.2902937515082110915592253119`です。sampleは約35時間15分で30件中27件が同一UTC日、ticker eligibleもちょうど30です。

default `fold_count=0`ではguardがsample/PBO条件でBLOCKED、PBOはNOT_ESTIMABLEです。現chainはcandidate/gate REJECT、kill/leaderboard KILL、strict v2 12-input packetが`BLOCKED_BY_BIAS_GUARD`です。専用PBO証跡がないため`COMPUTED_PASS`文字列だけでもREADYにはなりません。

slippage 2 bpsは`+3.04237 USD`、50 bpsは`-3.67763 USD`です。cost identity再計算は機能しますが、実約定摩擦への耐性は弱いままです。
...
event数だけで独立標本数を代用しない。PBOが`COMPUTED_PASS`でない場合、rolling stabilityやregime splitが名目上completeでも、良い候補ではなく追加データ・実計算・依存調整が必要な候補として読む。
...
1. 実PBO計算を実装し、production `COMPUTED_PASS`以外は進めない。
2. position overlap、資本拘束、同時保有上限をbacktest損益へ組み込む。
3. 時間的に離れたevent/episodeと複数regimeを増やし、always-long等の単純benchmarkを上回れるか確認する。
4. books / trades / replay とmeasured slippageの欠損を減らす。
5. cost identityとpack-local rows/guard lineageを維持し、別run artifactを混ぜない。
6. actual cashへ進む前に、estimateとactual cashの境界を再確認する。
