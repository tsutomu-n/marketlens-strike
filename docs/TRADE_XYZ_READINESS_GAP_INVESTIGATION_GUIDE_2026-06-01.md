...
この文書は、現在 `quote_coverage` が fail、`funding_events` と `oracle_timestamp_provenance` が known gap になっている理由を、第三者が調査できる粒度で分解する。
...
2. funding_events
   `funding_events_from_history` は usable だが、oracle quote join 漏れが残る。`skipped.missing_oracle_quote_within_lag` を確認し、quote coverage の連続性が伸びた後に再生成する。
