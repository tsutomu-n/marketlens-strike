...
README と `CURRENT_STATE.md` には、正本ではない判断補助として掲載済みです。公式説明ではなく、儲かった気になる前に止めるための判断メモです。
...
2026-06-21 の追記。下の command/doc count は当時の軽量確認値であり、現行 proof ではありません。現在値は `uv run python scripts/check_cli_catalog.py` と `uv run python scripts/check_current_docs.py` を再実行して確認します。

- `uv run python scripts/check_cli_catalog.py` は 205 public CLI commands。
- `uv run python scripts/check_current_docs.py` は 148 current docs。
- Crypto Perpには truth-cycle status、local-only next steps、stage checklist、fixture-only dogfood pack、Daily Brief / Workbench Viewer の blocker summary が増えた。
- これは読み合わせと誤読防止の改善であり、live readiness、network permission、exchange write permission、利益証明ではない。
