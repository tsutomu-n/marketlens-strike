...
- `data/reports/strategy_backtest_html_report.html`: ブラウザで開く report。損益グラフ、benchmark 比較、指定期間の取引一覧、stress summary を見られます。
- `data/research/backtest_html_report/strategy_backtest_html_report.json`: report の根拠 manifest。どの artifact を読んだか、結果ラベル、live を許可しない境界を記録します。

HTML の日付入力で期間を絞ると、その期間にどの signal / trade があり、合計 return がどう見えるかを確認できます。
...
標準入口は `strategy-backtest-html-report` です。
...
補足として、optional `quantstats` 経路で HTML report artifact を作る `strategy-backtest-report-extension` もあります。`strategy-backtest-html-report` は依存追加なしの repo 標準ビューア、`strategy-backtest-report-extension` は optional OSS report です。どちらも注文、wallet、signing、exchange write には接続しません。
