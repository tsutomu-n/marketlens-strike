...
2026-06-19_00:21 JST 時点での最終形は、次で固定する。
...
実装済みの次段 first slice:
  6. strategy_stage_policy.v1
  7. strategy-stage-policy-validate
  8. strategy_stage_decision.v1
  9. strategy-stage-decision
  10. strategy_paper_smoke_plan.v1
  11. strategy-paper-smoke-plan
  12. strategy_runtime_observation_manifest.v1
  13. strategy-runtime-observation-ingest
  14. paper_vs_backtest_drift_review.v1
  15. strategy-drift-review
  16. strategy_learning_event.v1
  17. strategy-learning-ledger-update
  18. strategy_revision_request.v1
  19. strategy-revision-request-build
  20. strategy_revision_request_review.v1
  21. strategy-revision-request-review
  22. strategy_authoring_update_handoff.v1
  23. strategy-authoring-update-handoff
  24. strategy_case_lite.v1
  25. strategy-case-lite-update
  26. strategy_daily_brief.v1
  27. strategy-daily-brief
  28. strategy_ai_review_packet.v1
  29. strategy_ai_review_note.v1
  30. strategy-ai-review-packet-build
  31. strategy-ai-review-note-record
  32. strategy_model_run.v1
  33. strategy_optimizer_trial_ledger.v1
  34. strategy-model-run-record
  35. strategy_micro_live_plan.v1
  36. strategy-micro-live-plan
  37. strategy_live_observation_manifest.v1
  38. strategy-live-observation-ingest
  39. strategy_scale_decision.v1
  40. strategy-scale-decision
  41. strategy_next_scale_plan.v1
  42. strategy-next-scale-plan
  43. strategy_workbench_viewer.v1
  44. strategy-workbench-viewer-build
```
...
実装を完了まで進めるために使った対象ファイル、テスト方針、完了条件は [archive/2026-06-22-doc-routing/STRATEGY_OPERATIONS_WORKBENCH_COMPLETION_PLAN_2026-06-19.md](archive/2026-06-22-doc-routing/STRATEGY_OPERATIONS_WORKBENCH_COMPLETION_PLAN_2026-06-19.md) に historical plan として残す。この target definition だけでは、コーダーが全 task を完了できる粒度ではない。
...
`strategy_input_contract.v1`、`strategy_input_contract_validation.v1`、`strategy_idea.v1`、`strategy_intake_decision.v1` と対応 CLI は first gate として実装済み。`strategy_stage_policy.v1`、`strategy_stage_policy_validation.v1`、`strategy_stage_decision.v1` と対応 CLI も Stage Policy / Decision first slice として実装済み。`strategy_paper_smoke_plan.v1`、`strategy_runtime_observation_manifest.v1`、`paper_vs_backtest_drift_review.v1`、`strategy_learning_event.v1`、`strategy_revision_request.v1`、`strategy_revision_request_review.v1`、`strategy_authoring_update_handoff.v1`、`strategy_case_lite.v1`、`strategy_daily_brief.v1`、`strategy_ai_review_packet.v1`、`strategy_ai_review_note.v1`、`strategy_model_run.v1`、`strategy_optimizer_trial_ledger.v1`、`strategy_micro_live_plan.v1`、`strategy_live_observation_manifest.v1`、`strategy_scale_decision.v1`、`strategy_next_scale_plan.v1`、`strategy_workbench_viewer.v1` と対応 CLI も paper smoke / runtime observation / drift review / learning / human review / authoring handoff / case timeline / daily brief / AI review / model loop / micro live plan gate / live observation / scale decision / next scale plan / static viewer first slice として実装済み。
...
| 現行実装に近い Strategy Review / Stage / Smoke / Runtime Observation / Drift / Learning slice | 93 / 100 | read-only review、operator decision、stage policy / decision、paper smoke plan、runtime observation ingest、PnL / cost を含む drift review、learning / revision request / authoring update handoff、case lite、daily brief、micro live plan gate、live observation ingest、scale decision、next scale plan、static viewer は first slice として有効。ただし Svelte UI と actual scale execution は未実装 |
| 旧い入力データ定義 | 55 / 100 | path / hash 中心で、`available_at`、execution reality、survivorship、revision policy が弱い |
| Strategy Input Contract 追加後の完成形定義 | 87 / 100 | 実務上の主な落とし穴を入口で止められる。first gate と optional column / timestamp / available_at validation は実装済みだが、runtime observation の自動反映は未実装 |
...
これらを束ねる first slice として、現行 repo には `strategy_input_contract.v1` と `strategy-input-contract-validate` がある。optional `validation_expectations` による required column、timestamp upper bound、available_at column の検査もできる。ただし、Paper Smoke、Runtime Observation Ingest、Drift Review まで同じ contract を強制参照させる接続はまだ後続 slice である。
...
最初の実装は、strategy factory よりも `strategy-intake-validate` と `strategy-input-contract-validate` を先に置く。この first gate は実装済み。戦略を増やすより、検証不能な候補を入口で落とす方が実務価値が高い。
...
現行 repo では、Idea Intake first slice として `strategy_idea.v1`、`strategy_intake_decision.v1`、`strategy-intake-validate` がある。古い [algo/strategy_factory/](algo/strategy_factory/) の docs と `Signal Candidate Sheet` は、発想整理の参考資料として読む。
...
最初に作るなら、`strategy-intake-validate` が最も価値が高い。この first gate は実装済み。候補シートの未記入、baseline なし、invalidation なし、required inputs なし、risk なしを機械で落とせるためである。
...
完成形では、基本形に加えて Human-in-the-loop、AI-in-the-loop、Multi-AI review loop、Model / Optimizer-in-the-loop を持てるようにする。
...
### Model / Optimizer-in-the-loop

Model / Optimizer-in-the-loop は、深層学習、決定木系 ML、統計 model、Bayesian optimization、遺伝アルゴリズムなどを使って、候補、特徴量、閾値、反証材料を作る loop である。

これは strategy adoption engine ではない。出力は `candidate` または `proposal` であり、採用、paper permission、micro live permission ではない。

使ってよい手法:

| 手法 | 使いどころ | 主なリスク |
|---|---|---|
| Linear / logistic model | baseline model、単純な確率推定、説明可能な比較対象 | 弱いが、比較対象として有用 |
| Decision tree / Random Forest / Gradient Boosting | regime 分類、entry filter、feature importance、失敗条件の発見 | 過学習、後付け filter、feature leakage |
| Deep learning | 大量データの表現学習、時系列 embedding、regime embedding | 個人運用ではデータ不足、検証不足、説明困難 |
| Bayesian optimization | parameter 探索の効率化 | search space と trial count を隠すと過剰最適化になる |
| Genetic algorithm | ルール候補、feature 組み合わせ、parameter 候補の探索 | overfitting 製造機になりやすい |
| Reinforcement learning | execution policy や simulator 研究 | 初期主軸にしない。live 実装には遠い |

優先順位:

```text
1. Linear / logistic baseline
2. Decision tree / Random Forest / Gradient Boosting
3. Bayesian optimization
4. Genetic algorithm
5. Deep learning
6. Reinforcement learning は最後
```

個人システムトレーダーの実務では、まず決定木系と simple model を使う方がよい。深層学習や遺伝アルゴリズムは、input contract、trial ledger、walk-forward、negative control、runtime observation がない段階で入れると、見栄えのよい過学習を作りやすい。

Model / Optimizer がやってよいこと:

- feature importance の候補を出す。
- regime filter の候補を出す。
- entry / exit threshold の候補を出す。
- no-trade condition の候補を出す。
- paper / runtime observation から失敗パターンを抽出する。
- backtest と paper の drift を説明する仮説を出す。
- negative control や baseline を追加提案する。
- revision request の候補を作る。

Model / Optimizer がやってはいけないこと:

- strategy spec を直接上書きする。
- 最良スコアの parameter を自動採用する。
- trial ledger を省略する。
- holdout / walk-forward / negative control なしに候補を進める。
- paper execution、micro live、position size 増加を許可する。
- live execution data と training data を境界なく混ぜる。

### Model Run Contract

Model / Optimizer を使う場合は、run contract を必ず残す。

```yaml
schema_version: strategy_model_training_run.v1
model_run_id: model-run-20260618-001
strategy_id: ndx-open-gap-residual
model_family: gradient_boosting
purpose: regime_filter_candidate
input_contract_id: ndx-open-gap-v001
training_data:
  path: data/model_inputs/...
  sha256: ...
  feature_snapshot_id: ...
  label_definition: next_session_direction_after_cost
split_policy:
  type: walk_forward
  train_periods: [...]
  validation_periods: [...]
...
leakage_guards:
  ...
  max_feature_source_ts_policy: <= decision_ts
search:
  search_space_hash: ...
  trial_count: 80
  seed: 42
...
Bayesian optimization や遺伝アルゴリズムでは、成功した candidate だけを残してはいけない。

必ず trial ledger を残す。

```yaml
schema_version: strategy_optimizer_trial_ledger.v1
optimizer_run_id: opt-run-20260618-001
strategy_id: ndx-open-gap-residual
method: genetic_algorithm
objective: maximize_cost_adjusted_expectancy_with_dd_penalty
search_space_hash: ...
trial_count: 500
seed: 42
kept_candidates: 5
rejected_trials: 495
overfit_warnings:
  - best candidate performance collapses on holdout
  - too many equivalent threshold variants
candidate_outputs:
  - candidate_id: ga-candidate-0042
    status: NEEDS_REVIEW
auto_applied: false
```

GA や optimizer の結果は、`strategy_model_candidate.v1` として Idea Intake または Revision Request へ戻す。直接 Strategy Authoring spec に反映しない。

### Model Review

Model / Optimizer の出力は、人間または AI review で読む。

Model review で見るもの:

- training data は `available_at` を守っているか。
- label に未来情報が混じっていないか。
- search space が広すぎないか。
- trial count が多すぎないか。
- holdout / walk-forward で崩れていないか。
- baseline に勝っているか。
- cost / slippage / no-fill を入れると消えないか。
- feature importance が market 仮説として説明できるか。
- runtime observation と矛盾していないか。
...
PROMOTE_TO_IDEA_INTAKE
PROMOTE_TO_REVISION_REQUEST
```

`PROMOTE_TO_IDEA_INTAKE` は backtest ready ではない。Model が出した候補を人間が検証入口へ送る、という意味だけである。
...
| `strategy_paper_smoke_plan.v1` | paper smoke 実行前の source / threshold / command preview |
...
| `strategy_model_training_run.v1` | model 学習 run の入力、split、metric、leakage guard |
| `strategy_model_candidate.v1` | ML / DL / optimizer が出した候補 |
| `strategy_optimizer_trial_ledger.v1` | Bayesian optimization / GA などの探索履歴 |
| `strategy_model_review.v1` | model output の人間 / AI review |
...
次の CLI は完成形候補を含む。`strategy-input-contract-validate`、`strategy-intake-validate`、`strategy-stage-policy-validate`、`strategy-stage-decision`、`strategy-paper-smoke-plan`、`strategy-runtime-observation-ingest`、`strategy-drift-review`、`strategy-learning-ledger-update`、`strategy-revision-request-build`、`strategy-revision-request-review`、`strategy-authoring-update-handoff` は実装済み。AI / model / case / daily brief 系は現行実装済みとは限らない。
...
uv run sis strategy-model-run-record \
  --strategy-id <strategy-id> \
  --model-run path/to/model_training_run.yaml \
  --out data/strategy_models

uv run sis strategy-optimizer-trial-ledger-validate \
  --ledger data/strategy_models/<strategy-id>/<optimizer-run-id>/optimizer_trial_ledger.yaml

uv run sis strategy-model-review-build \
  --strategy-id <strategy-id> \
  --model-run data/strategy_models/<strategy-id>/<model-run-id>/model_training_run.yaml \
  --out data/strategy_models/<strategy-id>/<model-run-id>/review
...
- model / optimizer candidate に未処理の overfit warning はあるか。
...
1. `strategy_input_contract.v1` と `strategy-input-contract-validate` 実装済み
2. `strategy_idea.v1` と `strategy-intake-validate` 実装済み
3. `strategy_stage_policy.v1` と policy validation 実装済み
4. `strategy_stage_decision.v1` と `strategy-stage-decision` 実装済み
5. Paper smoke plan / report 実装済み
6. Runtime Observation Ingest 実装済み
7. Paper vs Backtest Drift Review
8. Persistent Learning Loop / Revision Request / Authoring Update Handoff 実装済み
9. AI Review Packet / AI Review Note
10. Model / Optimizer-in-the-loop
11. Strategy Case Lite
12. Strategy Daily Brief
13. Micro Live Plan Gate
14. Micro Live Canary は最後。実行ではなく、まず計画と artifact contract から始める。

今すぐ UI、AI review、ML / DL / GA、Cynefin 詳細分類 UI を作るより、Strategy Input Contract を先に作る方が実務価値が高い。

理由は単純である。入力契約が弱いまま高度な loop を足すと、悪い入力、後付け仮説、過剰最適化、説明だけ上手い候補を増やす。最初に必要なのは、何を観測できて、何を観測できず、いつ利用可能だったデータで、どの execution reality を仮定したのかを固定することである。
...
- ML / DL / GA が overfitting 製造機になり、候補の見栄えだけを良くする。
- 最良 trial だけを残して、失敗 trial と探索回数を隠す。
...
- Model / Optimizer output も `auto_applied=false` を既定にし、training data、label、split、seed、search space、trial ledger、holdout result を必ず残す。
- Model / Optimizer candidate は Idea Intake または Revision Request へ戻し、Strategy Authoring spec へ直接反映しない。
...
- first gate として `strategy_input_contract.v1`、`strategy_input_contract_validation.v1`、`strategy_idea.v1`、`strategy_intake_decision.v1`、`strategy-input-contract-validate`、`strategy-intake-validate` は実装済み。
- Stage Policy / Decision first slice として `strategy_stage_policy.v1`、`strategy_stage_policy_validation.v1`、`strategy_stage_decision.v1`、`strategy-stage-policy-validate`、`strategy-stage-decision` は実装済み。
- Paper Smoke plan first slice として `strategy_paper_smoke_plan.v1` と `strategy-paper-smoke-plan` は実装済み。
- Runtime Observation Ingest first slice として `strategy_runtime_observation_manifest.v1` と `strategy-runtime-observation-ingest` は実装済み。
- Paper vs Backtest Drift Review first slice として `paper_vs_backtest_drift_review.v1` と `strategy-drift-review` は実装済み。
- Strategy Learning / Revision Request first slice として `strategy_learning_event.v1`、`strategy_revision_request.v1`、`strategy-learning-ledger-update`、`strategy-revision-request-build` は実装済み。
- Revision Request Review first slice として `strategy_revision_request_review.v1` と `strategy-revision-request-review` は実装済み。
- Authoring Update Handoff first slice として `strategy_authoring_update_handoff.v1` と `strategy-authoring-update-handoff` は実装済み。
- 次の実装候補は UI、strategy factory、AI review、ML / DL / GA、Cynefin 詳細分類 UI ではなく、Normal Paper Observation evidence contract を優先する。Strategy Review optional source connection は実装済み。
- completion plan は [archive/2026-06-22-doc-routing/STRATEGY_OPERATIONS_WORKBENCH_COMPLETION_PLAN_2026-06-19.md](archive/2026-06-22-doc-routing/STRATEGY_OPERATIONS_WORKBENCH_COMPLETION_PLAN_2026-06-19.md) に historical plan として archive 済み。この文書だけで実装完了まで閉じるとは扱わない。
