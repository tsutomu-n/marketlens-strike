...
- 次の実装候補は UI、strategy factory、AI review、ML / DL / GA、Cynefin 詳細分類 UI ではなく、まず `strategy_input_contract.v1` と `strategy-input-contract-validate` とする。
- Idea Intake は docs 運用だけでは足りないため、`strategy_idea.v1` と `strategy-intake-validate` を早めに足す。
- Stage Policy / Decision は重要だが、入力契約と Idea Intake の後に作る。入口が弱いまま gate だけ作っても、もっともらしい通過判定を増やすだけになる。
