...
`strategy-backtest-baseline-compare` は cash/no-trade、単純 momentum、単純 mean reversion、random throttle、funding carry skipped の baseline / negative control を `strategy_backtest_baseline_comparison.v1` に残します。単純 leverage は戦略リターン由来の diagnostic stress であり、独立 baseline の strongest / weakness 判定には含めません。
