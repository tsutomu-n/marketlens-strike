...
`strategy-backtest-artifact-summary` は pack、pack validation、benchmark relative、metric extension、report extension の主要 field を読み、JSON で stdout に出します。artifact を生成せず、欠損 artifact は `exists=false` として表示します。
