...
OSS を使った backtest capability expansion の実装済み計画は、実装前の外部調査や対象ファイル一覧を含む履歴資料として
...
に保存している。現行の optional framework / reference-only contract / Constraint Breaker 境界は
[BACKTEST_CURRENT_TECHNICAL_REFERENCE.md](BACKTEST_CURRENT_TECHNICAL_REFERENCE.md)、`pyproject.toml`、`uv.lock`、CLI help で確認する。
