...
`strategy-backtest-data-availability` は metrics / signals / quotes の local source hash、parquet row count、timestamp range、gap / duplicate count と、Bitget / Hyperliquid / Coinalyze future candidate 行を `backtest_data_availability_ledger.v1` に残します。外部 API は呼びません。
`strategy-backtest-baseline-compare` は cash/no-trade、単純 momentum、単純 mean reversion、random throttle、単純 leverage、funding carry skipped の baseline / negative control を `strategy_backtest_baseline_comparison.v1` に残します。
...
`strategy-backtest-execution-sim` は既存 Strategy Authoring metrics から paper-only order intents / fill events を作り、rate-limit / cancel-modify / unknown order state / market impact を未モデル仮定として `strategy_backtest_execution_simulation.v1` に残します。
