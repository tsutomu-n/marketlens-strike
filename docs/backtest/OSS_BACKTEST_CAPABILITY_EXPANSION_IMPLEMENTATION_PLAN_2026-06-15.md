...
2. dependency 追加なしで reference-only / metadata / contract を整えるもの:
   `HftBacktest`, `qstrader`, `skfolio`, `Freqtrade`, `NautilusTrader`, `Qlib`, `FinRL`
3. 現時点では採用しないもの:
...
- `HftBacktest`, `qstrader`, `skfolio`, `Freqtrade`, `NautilusTrader`, `Qlib`, `FinRL` は、最初は dependency 追加なしの reference-only / smoke / contract に閉じる。
...
| 10 | オプション | `NautilusTrader` | architecture reference | 強力だが統合が重い。標準 engine にしない。 |
| 11 | オプション | `LEAN` | external sandbox only | C# core / Python 3.11 前提があり、Python 3.13 repo 内統合に向かない。 |
| 12 | オプション | `Qlib`, `FinRL` | research sandbox only | ML / DRL platform で、Strategy Lab と責務が重なる。 |
| 13 | オプション | `backtesting.py`, `backtrader`, Zipline 系 | 原則 defer | license / build / live surface / maintenance risk が大きい。 |
...
`pyproject.toml` / `uv.lock` は、既存4 extras 以外の package を正式 optional extra にすると決めた時だけ変更する。`HftBacktest`, `qstrader`, `skfolio` の初期タスクでは変更しない。
...
### OBF7: skfolio portfolio validation contract を作る

goal:
`skfolio` を backtest engine と誤認せず、portfolio validation / cross-validation / stress-test の候補として contract 化する。
...
- `skfolio` は reference-only として扱い、engine_run=false を維持する。
...
### OBF8: Operator docs と current docs を更新する
...
OBF1 から OBF3 が必須完了範囲である。OBF4 から OBF7 は推奨範囲で、dependency 追加なしの設計・readiness・contract までを完了線にする。OBF9 はどの範囲で止めても最後に実行する。
...
- `qstrader` と `skfolio` は、dependency 追加前の contract artifact まで進み、engine 実行可能と誤読されない。
