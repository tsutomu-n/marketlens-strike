...
確認結果が `pass` なら、今回使った local data はその検査では大きく壊れていない、という意味です。
...
状態は、生成済み artifact と再実行したコマンドの結果で変わります。固定の表を読むのではなく、次を実行して今の値を見ます。

| 見るもの | 確認方法 | かんたんな意味 |
|---|---|---|
| pack があるか | `uv run sis strategy-backtest-artifact-summary` | まとめ結果が作られているか |
| pack validation | `uv run sis strategy-backtest-pack-validate` | まとめ結果の形と安全境界が通っているか |
| suite methods / runs | artifact summary の suite 関連 field | 複数の試し方を使っているか |
| paper only | artifact summary の `paper_only` 系 field | 研究・紙上取引用に閉じているか |
| permits live order | artifact summary の live order 系 field | 本物の注文を許可していないか |
| wallet / exchange write | artifact summary の wallet / exchange write 系 field | wallet や取引所書き込みを使っていないか |
| backtest acceptance | `strategy-backtest-acceptance` の出力 | バックテスト段階の判定 |
| paper observation | `uv run sis strategy-paper-observation-status` | paper observation を続けるべきか |
...
状態確認で paper observation 継続が出る場合は、次のように読みます。

```text
バックテストは通った
-> paper observation に進む材料はある
-> でも paper observation の観察数がまだ足りない
-> だから paper observation を続ける
```
