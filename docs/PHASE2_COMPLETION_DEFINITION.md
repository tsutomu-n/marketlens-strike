...
- Phase 2 の `signals.csv` は「研究用に生成可能」であればよい。
- QQQ baseline strategy 自体の最終確定は Phase 4 の責務。
- ただし Phase 2 完了判定では、feature panel を入力に最低 1 本の signal CSV を生成できる必要がある。
...
`build-backtest` は既存 CLI なので、Phase 2 完成条件には含めない。signal-driven backtest の完成は Phase 4 の責務とする。
