...
| `docs/TRADE_XYZ_IMPLEMENTATION_STATUS_AUDIT_2026-05-28.md` | current | Trade[XYZ] 実装状況の current audit。P2 gate restore 後の値へ更新済み | `checked_files=12`, `292 passed`, fee mode resolved, Alpaca non-stub を current truth として扱う |
...
| `data/reports/weekly_strategy_review.md` | generated, guarded | QQQ/SPY/XAU など old symbols は historical/backtest input として表示し、先頭に current Trade[XYZ] gate snapshot を出す。current symbol universe は `diagnostics_symbols` を読む |
...
- `checked_files=12`, `292 passed`, `READ_ONLY_GO`, `P2_BLOCKER=0`, `LIVE_READINESS_BLOCKER=6` を反映する。
...
- only live-readiness blockers の場合、phase gate remediation order は `none` になる。
...
- `uv run sis alpaca-smoke` は operator entry として存在し、`pass` / `blocked` / `failed` のどれでも summary / report を残す。
- live bars が返っても `source_confidence` が低い場合は `status=blocked` とする。

Better:

- credentials がある環境で `uv run sis alpaca-smoke --symbol NVDA --timeframe 15m --limit 1 --feed iex` を実行し、`status=pass` artifact を保存する。
...
`data/reports/*.md` は regenerated snapshot であり、old decision が混ざる場合がある。

`data/reports/weekly_strategy_review.md` は 2026-05-28 時点で current Trade[XYZ] gate snapshot を先頭に出す。`Backtest Metrics Snapshot` に `QQQ/SPY/XAU` が出る場合は historical/backtest input であり、current Trade[XYZ] target universe ではない。
...
1. credentials がある環境で `uv run sis alpaca-smoke --symbol NVDA --timeframe 15m --limit 1 --feed iex` を実行し、Alpaca live smoke を `status=pass` にする。ただし secrets は repo に書かない。
...
- `uv run sis alpaca-smoke` を operator entry として追加し、credentials なしでも failed summary / report を残すようにした。
- `data/reports/weekly_strategy_review.md` の先頭に current Trade[XYZ] gate snapshot を追加し、old symbols を historical/backtest input と明示した。
- `Paper Last Run Phase Gate` と current `phase_gate_review_summary.json` の差分を、weekly review 内で last-run snapshot と current snapshot として明示した。
- phase gate remediation order から live-readiness-only drift を外し、P2 remediation loop と live-readiness blocker を分離した。
