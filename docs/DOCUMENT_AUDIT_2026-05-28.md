...
| `docs/TRADE_XYZ_IMPLEMENTATION_STATUS_AUDIT_2026-05-28.md` | current | Trade[XYZ] 実装状況の current audit。P2 gate restore 後の値へ更新済み | `checked_files=12`, `291 passed`, fee mode resolved, Alpaca non-stub を current truth として扱う |
...
- `checked_files=12`, `291 passed`, `READ_ONLY_GO`, `P2_BLOCKER=0`, `LIVE_READINESS_BLOCKER=6` を反映する。
...
- `uv run sis alpaca-smoke` は operator entry として存在し、成功・失敗どちらでも summary / report を残す。
...
2. `paper_last_run` 由来の `Paper Last Run Phase Gate` と current `phase_gate_review_summary.json` の差分を、weekly review 内で last-run snapshot と current snapshot としてさらに明示する。
