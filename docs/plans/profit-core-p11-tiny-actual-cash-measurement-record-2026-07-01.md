...
## 追加監査 hardening: 2026-07-01_17:53 JST

追加調査で、P11 が `candidate_id` と合計値だけで完了扱いになる余地を確認した。以下を complete 条件に追加する。

- P9 readiness packet と P10 external venue adapter は expected schema version を持つ。
- order intent、submitted order、fills、fee/funding、flat reconciliation、stop condition の `event_id` は actual-cash rows の event set に一致する。
- submitted order は order intent id を参照し、fills は submitted order id を参照する。
- order intent の action は、同一eventの measured non-`NO_TRADE` row と一致する。
- P10 adapter venue と submitted order venue は一致する。
- actual-cash rows は event ごとに measured non-`NO_TRADE` action を1つだけ持つ。
- cash ledger の event set は actual-cash rows の event set と一致し、未割当 `event_id=null` entry を含まない。
- fee/funding evidence の fee / funding totals は cash ledger の `FEE` / `FUNDING` entries と一致する。

追加失敗条件:

- 別eventのfills、別orderのfills、別venueのsubmitted order、別eventまたは未割当entryを含むcash ledger、fee/funding total不一致、非actual-cash order intent、または複数trade action rows が complete 扱いになる。
