...
| `docs/plans/*.md` from 2026-07-04/05 | 完了済み implementation plan。branch 名や当時 artifact を含む | archive。ただし今回作成した cleanup plan は作業中 record として残す |

## 作り直したほうがいいドキュメント

| doc | 理由 | 今回の処置 |
|---|---|---|
| `docs/APP_CURRENT_STATE_DETAILED_2026-06-20.md` | 非技術者向け説明、技術 detail、surface map が混ざる | 今回は維持。次回は overview / glossary / technical reference に分割 |
| `docs/final-summary.md` | addendum が増え、current proof と history ledger が混ざる | 今回は維持。current proof ではなく履歴 ledger として扱う |
...
- `APP_CURRENT_STATE_DETAILED` と `final-summary` は今回 archive しないが、次に大きく触るなら分割したほうがよい。
