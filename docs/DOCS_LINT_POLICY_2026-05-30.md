...
`scripts/check_current_docs.py` が検査する対象は script の `CURRENT_DOC_FILES` と `CURRENT_DOC_DIRS` を正とする。代表的な入口は次です。
...
- primary current docs は、`docs/archive/README.md` と `plan/README.md` 以外の archive 直リンクを増やさない。
...
- tracked plan file は `plan/README.md`、`plan/2026-06-22-strategy-feedback-case-index/README.md`、`plan/archive/**` に限定する。root 側に current unimplemented plan を再追加する場合は、実装状態と archive 対象を確認してから checker の allowlist を更新する。
