...
- tracked plan file は `plan/README.md` と `plan/archive/**` だけに限定する。root 側に current unimplemented plan を再追加する場合は、実装状態と archive 対象を確認してから checker の allowlist を更新する。
