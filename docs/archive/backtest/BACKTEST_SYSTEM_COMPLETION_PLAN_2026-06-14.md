...
| Baseline comparison | `strategy-backtest-baseline-compare` | `strategy_backtest_baseline_comparison.v1` | cash/no-trade と、実行済み return series 由来の control を記録。leverage は独立 baseline ではなく diagnostic stress として分離 |
...
- `data_availability` の group-aware gap / duplicate は local parquet に存在する列だけを key に使う。将来 venue schema を広げる時は明示 key contract を追加する。
