...
2026-06-17_20:14 JST の status run:
...
latest_normal_requirement_gaps.fills=20/20 remaining=0
...
- 現行の local one-shot cycle / append は1回につき `paper_fills=1` を追加する。最新通常 session は `fills=20/20` まで到達したが、`trading_days=1/10` なので通常passの証明にはまだ足りない。
