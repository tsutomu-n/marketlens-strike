...
# solana-developers/program-examples: A repository of Solana program examples
![](https://opengraph.githubassets.com/7ea1b2e4214282bc73b628ea4b62e49e66d83145e3d3ac5be62150725f296789/solana-developers/program-examples)


[Source URL](https://github.com/solana-developers/program-examples)

- このリポジトリには、Solana のオンチェーンプログラム（他のブロックチェーンでは「スマートコントラクト」と呼ばれる）の例が含まれています。
- このリポジトリには、Anchor、Native Rust、TypeScript、Python での Solana 開発の例が含まれています。
- このリポジトリには、Solana 開発のためのさまざまなフレームワークの例が含まれています。たとえば、Anchor、Poseidon、Seahorse などです。
- それぞれのフォルダーには、Anchor、Native Rust、Poseidon、Seahorse での開発の例が含まれています。
- Anchor では、Rust を使用して開発を行います。プログラムをビルドしてデプロイするには、`anchor build && anchor deploy` を実行します。テストするには、`anchor run test` を実行します。
- Native Rust では、Solana のネイティブ Rust クレートとバニラ Rust を使用して開発を行います。プログラムをビルドしてデプロイするには、`cicd.sh` を実行します。テストするには、`yarn run test` を実行します。
- Poseidon では、TypeScript コードを Anchor Rust に変換して開発を行います。
- Seahorse では、Python コードを Anchor Rust に変換して開発を行います。プログラムをビルドしてデプロイするには、`seahorse build && anchor deploy` を実行します。テストするには、`anchor run test` を実行します。
- このリポジトリには、トークン拡張、圧縮、NFT などのさまざまなプログラムの例が含まれています。
- トークン拡張の例には、トークン ミントの作成、トークンの Mint、トークンの転送などが含まれています。
- 圧縮の例には、圧縮 NFT のバーン、圧縮 NFT のストレージなどが含まれています。
- NFT の例には、NFT の作成、NFT の転送などが含まれています。
- このリポジトリは、Solana 開発者がさまざまなプログラムの例を参考にして開発を行うことを目的としています。
# Program Examples
