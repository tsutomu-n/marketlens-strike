...
1. `edge_candidate_factory` package namespaceだけを作る。
2. 将来task用の空moduleや空public commandは作らない。
3. CLI registrationはT4以降、実体があるcommandごとに追加する。
4. `scripts/check_current_docs.py` は原則変更しない。新docsが `docs/plans` 配下なら既に対象です。
...
9. public commandはこのtaskで `edge-candidate-factory-run` だけを登録する。
10. `edge-candidate-backtest-kill-gate` はT6、`edge-candidate-artifact-summary` はT11まで登録しない。
...
- `uv run sis --help` と `uv run python scripts/check_cli_catalog.py` が通る。
...
既存backtest artifactから読める値と読めない値を混ぜない。`backtest_inputs.py` は最低限次を読む。

```text
strategy_backtest_pack.json
strategy_backtest_pack_validation.json
strategy_authoring_backtest_metrics.json or equivalent metrics artifact
strategy_backtest_suite_result.json
strategy_backtest_baseline_comparison.json
strategy_backtest_stress.json
strategy_backtest_data_availability_ledger.json
strategy_backtest_no_lookahead_diff.json
strategy_backtest_assumption_ledger.json
strategy_backtest_trial_ledger.json
```

抽出できないmetricは推定で埋めない。`metric_status=NOT_ESTIMABLE` と `INCONCLUSIVE_DATA` に落とす。
...
条件はbooleanだけにしない。各conditionは次を持つ。

```text
condition_id
condition_status = PASS | FAIL | NOT_ESTIMABLE | NOT_APPLICABLE
observed
required
source_ref
```
...
- `largest_loss_usd`, `profit_concentration`, `after_cost_edge_over_no_trade_usd` などが既存artifactから抽出不能なら `SHORTLIST_FOR_VIRTUAL` にしない。
- public command `edge-candidate-backtest-kill-gate` はこのtaskで初めて登録する。
...
## T9: Risk-Taker / Actual Cash handoff contract v0

### 目的

既存 `crypto-perp-risk-taker-review` と `crypto-perp-actual-cash-report-gate` へ進める前に、virtual/backtest evidence と actual-cash evidence を混同しない handoff を作る。
...
`edge_candidate_risk_actual_cash_handoff.v1` に次を保存する。
...
risk_taker_review_input_status = BLOCKED_NEEDS_ACTUAL_CASH_ROWS | READY_WITH_ACTUAL_CASH_ROWS
actual_cash_report_gate_input_status = BLOCKED_NEEDS_ACTUAL_CASH_ROWS | READY_WITH_ACTUAL_CASH_ROWS
...
1. `crypto-perp-risk-taker-review` へ進むには、既存 `crypto_perp_tournament_rows.v2` 互換rows、source availability、bias guard、actual cash availabilityの有無を明示する。
2. `crypto-perp-actual-cash-report-gate` へ進むには、既存 actual-cash rows builder が作る actual-cash rows が必須。
3. `virtual_execution_gate.v1` と `backtest_kill_gate.v1` はsource refsに残せるが、actual cash rowsの代替にしない。
4. actual cash rowsが無い場合は `BLOCKED_NEEDS_ACTUAL_CASH_ROWS` を正式結果にする。
...
## T10: LLM Adversarial Evidence Review v0
...
## T11: Integration summary command
