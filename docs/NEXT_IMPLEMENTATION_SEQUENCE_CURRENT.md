...
2026-06-17_19:52 JST の status run:
...
latest_normal_requirement_gaps.fills=1/20 remaining=19
latest_normal_requirement_gaps.trading_days=1/10 remaining=9
latest_normal_requirement_gaps.timestamp_quality=complete
...
- `normal_session_count` は通常sessionの件数であり、通常thresholdの合格条件ではない。
- 通常pass は、最新の通常sessionの review decision が `PASS_PAPER_OBSERVATION_REVIEW` になった時だけ成立する。
- 不足量は top-level の `latest_normal_requirement_gaps` を見る。
- 現行の local one-shot cycle は `paper_fills=1` を生成する。通常thresholdは `min_fills_for_pass=20` / `min_trading_days_for_pass=10` なので、短時間に同じlocal cycleを回すだけでは通常passの証明にならない。
- 既存 `--session-id` の再利用は fail closed する。既存 session に追記して長期観察の代用にしない。
