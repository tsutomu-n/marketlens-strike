...
the JSON manifests. The current default fixture has 90 feature rows and 84 residual rows, so the
current expected decision is `APPROVE_STRATEGY_LAB_EXPORT` for the Layer 2.5 research-only export
bridge.
