...
- `crypto-perp-risk-taker` profile の shortlist は C9 v0 の directional authoring bridge 境界に合わせ、`side_bias=long|short` だけを通す。`both` / `no_trade` は candidate inventory と search ledger に `REJECTED` として残す。
- `crypto-perp-risk-taker` profile の bridge-first shortlist は、現 C9 v0 public source で必要 source が無い `perp_reversal_after_liquidation_move` と `perp_open_interest_liquidation_pressure` を `REJECTED` として残す。`liquidation_notional` や `open_interest` は推定 proxy で埋めない。
...
- `strategy-idea-candidates-bitget-source-refresh` は Bitget public REST の contracts / tickers / 5m history candles から `--out/source_root/` に C9 bridge 互換 source root を作る。
