...
- `strategy-micro-live-plan`、`strategy-scale-decision`、`strategy-next-scale-plan` の `READY_FOR_HUMAN_*` stdout は `status=needs_human_approval`、`requires_explicit_approval=true`、`permits_live_order=false` と読む。`status=pass` ではない。blocked path は `status=blocked`、`requires_explicit_approval=false`、`permits_live_order=false` と読む。
...
- CLI stdout の ready-human path は `status=needs_human_approval`、`requires_explicit_approval=true`、`permits_live_order=false`。blocked path は `status=blocked`、`requires_explicit_approval=false`、`permits_live_order=false`。
- paper order、live order、wallet、signing、exchange write は実行しない。
