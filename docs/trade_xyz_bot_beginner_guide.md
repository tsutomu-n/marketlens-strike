...
いま使えるものと、まだ通常 bot 化していないものを分けて読みます。

- 使える: bot 用の判断メモを、人間が読みやすい形で出す。
- 使える: 「取引しない理由」を明確に残す。
- 制約あり: 注文候補は通常運用として出さず、v1 は HOLD preview に限定する。
- 制約あり: 銘柄リスト、価格、現実市場、比較、判定をまとめて更新する単一入口はまだ標準化しない。
...
4. 判断メモ: `uv run sis bot-preview` で `bot_decision.json` に「今は取引しない」と理由を書く。
