...
collector は稼働中で、quote coverage は伸びている。ただし、30日coverageと oracle timestamp provenance が未充足である。`real_market_reference` は現在の readiness manifest では pass である。
...
real market reference は現在の readiness manifest では pass である。5/30以前のreference dataをarchiveした直後は、`--start 2026-05-31 --end 2026-06-03` の再取得で株式/ETF/index proxyの多くが返らず fail だったが、その状態は現在値ではない。
...
data/manifests/trade_xyz_real_market_reference_manifest.json:
  ...
  row_count: 3782
  returned_symbols:
    AAPL, AMD, AMZN, EURUSD=X, EWJ, GOOGL, META, MSFT, NVDA, QQQ, SPY, TSLA, USDJPY=X, UUP, ^VIX
