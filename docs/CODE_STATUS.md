...
| Strategy backtest pack | DONE / standard local artifact chain with validation | `src/sis/backtest/pack.py`, `schemas/strategy_backtest_pack.v1.schema.json`, `schemas/strategy_backtest_pack_validation.v1.schema.json`, `strategy-backtest-pack`, `strategy-backtest-pack-validate`, `tests/strategy_authoring/test_cli_bundle.py`; runs single Strategy Authoring backtest, five-method suite, bundle result, adapter spike, external result, portfolio comparison, metric extension, comparison, writes pack manifest/report with artifact paths, hashes, and `external_framework_policy`, and validates path/hash/method/boundary/framework-policy checks while preserving paper-only / no-live boundaries and `locked_dependency_added=false` |
...
- `strategy-backtest-adapter-selection` は adapter spike と framework smoke の artifact から `strategy_backtest_adapter_selection.v1` artifact と report を作る。2026-06-13_17:16 JST 時点では `vectorbt`, `bt`, `empyrical-reloaded` を selected、その他6候補を deferred として記録する。`pyproject.toml` / `uv.lock` は変更せず、external engine は実行せず、live order、wallet、exchange write は許可しない。
- `strategy-backtest-adapter-contract` は `strategy_backtest_adapter_selection.v1` から `strategy_backtest_adapter_contract.v1` artifact と report を作る。`vectorbt`, `bt`, `empyrical-reloaded` の入力、出力、provenance、受入条件を記録する。`pyproject.toml` / `uv.lock` は変更せず、external engine は実行せず、live order、wallet、exchange write は許可しない。
...
- `strategy-backtest-pack` は標準 backtest artifact chain を一括生成する。単発 Strategy Authoring backtest metrics、5手法 suite、bundle result、adapter spike、external result、portfolio comparison、metric extension、comparison、pack manifest/report を作り、`strategy_backtest_pack.v1` に artifact path / hash、suite method count、external engine 実行有無、comparison id、`external_framework_policy` を記録する。標準 engine は `strategy_authoring_native`、標準完成線は `complete_without_locked_external_dependency` で、これは live order、wallet、exchange write を許可しない。
