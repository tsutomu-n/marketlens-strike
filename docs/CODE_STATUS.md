...
| Strategy backtest external result | DONE / dependency-free optional runner surface | `src/sis/backtest/external.py`, `schemas/strategy_backtest_external_result.v1.schema.json`, `strategy-backtest-external-run`, `tests/strategy_authoring/test_backtest_external.py`; records external framework run status, metrics slot, metrics/signals/quotes source hashes, and horizon while preserving dependency_added=false and live disabled; calls `vectorbt.Portfolio.from_signals` when `vectorbt` is installed and signals / quotes inputs are present; temporary `uv run --with vectorbt` smoke completed |
| Strategy backtest pack | DONE / standard local artifact chain with validation | `src/sis/backtest/pack.py`, `schemas/strategy_backtest_pack.v1.schema.json`, `schemas/strategy_backtest_pack_validation.v1.schema.json`, `strategy-backtest-pack`, `strategy-backtest-pack-validate`, `tests/strategy_authoring/test_cli_bundle.py`; runs single Strategy Authoring backtest, five-method suite, adapter spike, external result, comparison, writes pack manifest/report with artifact paths, hashes, and `external_framework_policy`, and validates path/hash/method/boundary/framework-policy checks while preserving paper-only / no-live boundaries and `locked_dependency_added=false` |
...
- `strategy-backtest-compare` は `strategy_backtest_metrics.json` から `strategy_backtest_comparison.v1` artifact と report を作る。既定の suite result が存在する場合は `suite_results`、既定の adapter spike が存在する場合は `adapter_spike`、既定の external result が存在する場合は `external_results` として取り込み、threshold failure、weakest era、suite best run を `comparison_diagnostics` に記録する。suite の `method_matrix` と run ごとの `method_id` も保持する。外部 framework 候補の installed 状態も記録するが、live 許可は出さず、`permits_live_order=false` を維持する。
...
- `strategy-backtest-external-run` は external framework 候補の run status を `strategy_backtest_external_result.v1` artifact と report に記録する。`vectorbt` がインストール済みで signals / quotes 入力がある場合は `vectorbt.Portfolio.from_signals` を呼ぶ。`uv run --with vectorbt` の一時 smoke では `vectorbt_version=1.0.0` で `engine_run=true`、`trade_count=7` を確認済み。現環境で framework が未インストールなら `skipped/not_installed_in_current_env` を記録し、`pyproject.toml` / `uv.lock` は変更せず、live order、wallet、exchange write は許可しない。
- `strategy-backtest-pack` は標準 backtest artifact chain を一括生成する。単発 Strategy Authoring backtest metrics、5手法 suite、adapter spike、external result、comparison、pack manifest/report を作り、`strategy_backtest_pack.v1` に artifact path / hash、suite method count、external engine 実行有無、comparison id、`external_framework_policy` を記録する。標準 engine は `strategy_authoring_native`、標準完成線は `complete_without_locked_external_dependency` で、これは live order、wallet、exchange write を許可しない。
