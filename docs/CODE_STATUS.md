...
| Bitget / Hyperliquid venue capability gate | DONE / fixture-first capability contract | `src/sis/venues/capabilities.py`, `tests/test_venue_capabilities.py`, `docs/venues/bitget_hyperliquid_capability_gate.md` |
...
- Venue capability は `src/sis/venues/capabilities.py` が正本。`bitget_futures` と `hyperliquid_perp` は known future venues だが、現行では schema / paper / network / live が disabled。
