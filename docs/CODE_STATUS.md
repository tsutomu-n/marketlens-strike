...
| Strategy backtest portfolio comparison | DONE / dependency-free optional bt runner surface | `src/sis/backtest/portfolio_comparison.py`, `schemas/strategy_backtest_portfolio_comparison.v1.schema.json`, `strategy-backtest-portfolio-compare`, `tests/strategy_authoring/test_backtest_portfolio_comparison.py`; records `bt` portfolio allocation / rebalance comparison status, source bundle hash, price frame hash, framework version, runner mode, portfolio return, drawdown, turnover, and rebalance count while preserving dependency_added=false and live disabled; temporary `uv run --with bt` smoke completed |
| Strategy backtest metric extension | DONE / dependency-free optional empyrical metrics surface | `src/sis/backtest/metric_extension.py`, `schemas/strategy_backtest_metric_extension.v1.schema.json`, `strategy-backtest-metric-extension`, `tests/strategy_authoring/test_backtest_metric_extension.py`; records `empyrical-reloaded` metrics normalization status, source backtest metrics hash, returns series hash, framework version, runner mode, Sharpe / drawdown / annual return / annual volatility slots while preserving dependency_added=false and live disabled; temporary `uv run --with empyrical-reloaded` smoke completed |
| Strategy backtest report extension | DONE / dependency-free optional quantstats report surface | `src/sis/backtest/report_extension.py`, `schemas/strategy_backtest_report_extension.v1.schema.json`, `strategy-backtest-report-extension`, `tests/strategy_authoring/test_backtest_report_extension.py`; records `quantstats` report status, source backtest metrics hash, returns series hash, framework version, runner mode, HTML report path/hash, and metrics table row count while preserving dependency_added=false and live disabled; temporary `uv run --with quantstats` smoke completed |
| Strategy backtest pack | DONE / standard local artifact chain with validation | `src/sis/backtest/pack.py`, `schemas/strategy_backtest_pack.v1.schema.json`, `schemas/strategy_backtest_pack_validation.v1.schema.json`, `strategy-backtest-pack`, `strategy-backtest-pack-validate`, `tests/strategy_authoring/test_cli_bundle.py`; runs single Strategy Authoring backtest, five-method suite, bundle result, adapter spike, external result, portfolio comparison, metric extension, report extension, comparison, writes pack manifest/report with artifact paths, hashes, and `external_framework_policy`, and validates path/hash/method/boundary/framework-policy checks while preserving paper-only / no-live boundaries and `locked_dependency_added=false` |
...
- `strategy-backtest-portfolio-compare` は `bt` 用の portfolio allocation / rebalance comparison を `strategy_backtest_portfolio_comparison.v1` artifact と report に記録する。通常 locked env で `bt` が未インストールなら `skipped/not_installed_in_current_env` を記録する。`uv run --with bt` の一時 smoke では `framework_version=1.2.0`, `runner_mode=temporary_or_optional_import`, `engine_run=true`, `portfolio_return=0.005833333333333801`, `rebalance_count=1` を確認済み。`pyproject.toml` / `uv.lock` は変更せず、live order、wallet、exchange write は許可しない。
- `strategy-backtest-metric-extension` は `empyrical-reloaded` 用の metrics normalization を `strategy_backtest_metric_extension.v1` artifact と report に記録する。通常 locked env で `empyrical` が未インストールなら `skipped/not_installed_in_current_env` を記録する。`uv run --with empyrical-reloaded` の一時 smoke では `framework_version=0.5.12`, `runner_mode=temporary_or_optional_import`, `metric_status=completed`, `engine_run=true`, `return_count=7`, `sharpe_ratio=7684.451501905242`, `max_drawdown=0.0`, `annual_return=0.18229407031297362`, `annual_volatility=0.000021798866106592272` を確認済み。`pyproject.toml` / `uv.lock` は変更せず、live order、wallet、exchange write は許可しない。
- `strategy-backtest-report-extension` は `quantstats` 用の report / tear sheet を `strategy_backtest_report_extension.v1` artifact と report に記録する。通常 locked env で `quantstats` が未インストールなら `skipped/not_installed_in_current_env` を記録する。`uv run --with quantstats` の一時 smoke では `framework_version=0.0.81`, `runner_mode=temporary_or_optional_import`, `report_status=completed`, `engine_run=true`, `return_count=7`, HTML report path/hash、metrics table row count を確認済み。`pyproject.toml` / `uv.lock` は変更せず、live order、wallet、exchange write は許可しない。
- `strategy-backtest-pack` は標準 backtest artifact chain を一括生成する。単発 Strategy Authoring backtest metrics、5手法 suite、bundle result、adapter spike、external result、portfolio comparison、metric extension、report extension、comparison、pack manifest/report を作り、`strategy_backtest_pack.v1` に artifact path / hash、suite method count、external engine 実行有無、comparison id、`external_framework_policy` を記録する。標準 engine は `strategy_authoring_native`、標準完成線は `complete_without_locked_external_dependency` で、これは live order、wallet、exchange write を許可しない。
