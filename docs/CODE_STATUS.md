...
| Strategy backtest adapter spike | DONE / dependency-free metadata spike | `src/sis/backtest/adapter_spike.py`, `src/sis/backtest/frameworks.py`, `schemas/strategy_backtest_adapter_spike.v1.schema.json`, `strategy-backtest-adapter-spike`, `tests/strategy_authoring/test_backtest_adapter_spike.py`; records external framework candidate metadata, license blockers, and adoption next step without dependency addition or engine execution |
...
- `strategy-backtest-adapter-spike` は external framework 候補の import / metadata / license risk / adoption blocker を `strategy_backtest_adapter_spike.v1` artifact と report に記録する。`pyproject.toml` / `uv.lock` は変更せず、external engine は実行せず、live order、wallet、exchange write は許可しない。
