...
用途: native backtest、suite、benchmark、stress、data availability、baseline、no-lookahead、execution simulation、assumption ledger、comparison、pack validation を一括で確認する。
...
現行 repo には `strategy-backtest-report-extension` があり、optional `quantstats` 経路で HTML report artifact を扱えます。ただし、ユーザーが指定期間の trade、損益グラフ、チャートを対話的に見るための統一 HTML / JS dashboard はまだ標準入口ではありません。

その機能を追加する場合も、最初は existing backtest artifacts を読む read-only renderer とし、live order、wallet、signing、exchange write に接続しない。
